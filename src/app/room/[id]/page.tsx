...
import { Copy, Check, Crown, Users, Play, Zap, Sparkles, UserPlus, AlertCircle, Loader2 } from "lucide-react";
...
export default function RoomPage() {
  ...
  const { room, players, loading, error, updateRoomStatus, submitWord, assignWords } = useRoom(roomCode);
  ...
  const handleJoinRoom = useCallback(async (name?: string) => {
    ...
    setJoinError(null);
    try {
      const { player } = await joinRoom(roomCode, nameToUse);
      setCurrentPlayerId(player.id);
      localStorage.setItem(`player_id_${roomCode}`, player.id);
      localStorage.setItem(`player_name_${roomCode}`, nameToUse);
      // Don't set hasJoined here - let the useEffect handle it when player appears in real-time data
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-rose-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-indigo-900 font-bold text-lg">Loading room...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !room) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-rose-100 flex items-center justify-center p-4">
        <Card className="max-w-md">
          ...
          <CardContent className="space-y-4">
            <p className="text-slate-600">
              {error || "This room doesn't exist. Please check the room code and try again."}
            </p>
          ...
          <Card className="bg-white/90 backdrop-blur-xl border-[3px] border-indigo-950 shadow-[0px_12px_0px_0px_#1e1b4b]">
            ...
            <CardContent className="p-6 md:p-8 space-y-8">
              ...
              <div className="pt-6 border-t-[3px] border-indigo-100 border-dashed">
                {joinError && (
                  <div className="mb-4 p-4 bg-rose-50 border-2 border-rose-300 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                    <AlertCircle className="w-5 h-5 text-rose-600 flex-shrink-0" />
                    <p className="text-rose-700 font-bold text-sm">{joinError}</p>
                  </div>
                )}
